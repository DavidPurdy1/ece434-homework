...
static int flash2(void *arg){
   ...
   printk(KERN_INFO "EBB LED 2: Thread has run to completion \n");
   return 0;
}
...
static int __init ebb_LED_init(void){
   ...
   gpio_request(gpioLED2, "sysfs");          // gpioLED is 60 by default, request it
   ...
   task2 = kthread_run(flash2, NULL, "LED_flash_thread2");  // Start the LED flashing thread 2
   ...
   if(IS_ERR(task2)){                                     // Kthread name is LED_flash_thread
      printk(KERN_ALERT "EBB LED: failed to create the task 2\n");
